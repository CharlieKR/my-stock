...
My 전체 자산 카드의 누적 수익금·수익률은 자산에 포함된 정산일의 `cumulativePnl`·`cumulativeReturn`을 사용합니다. 전체 누적 수익금은 투자별 누적 손익을 자산과 같은 기준환율로 원화 환산한 합계이고, 전체 누적 수익률은 이 합계를 원화 원금(전체 자산 − 누적 손익 합계)으로 나눈 값입니다. 아래 기간별 성과 계산과 구분하며, 투자별 정산일을 함께 표시합니다. 누적 손익이 누락된 투자 기록은 0으로 간주하지 않습니다.
